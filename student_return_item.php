...
if ($_SERVER["REQUEST_METHOD"] == "POST") {	
	...
	if ($error_msg == ""){
		$sql = "UPDATE activity SET date_ret='". $date_ret. "', teacher_appr = 1 WHERE user_id = '".$_SESSION["user_id"]."' and item_id = ".$_POST["item_id"] . " and date_ret = '0000-00-00 00:00:00'";
...
<!--
				<form action="<?php if($error_msg ==""){echo htmlspecialchars(basename($_SERVER['REQUEST_URI']));} ?>" method="post">

				<label>Item ID: <u><?php if($error_msg ==""){echo $row['item_id']; }?></u> </label>
			    	<BR><BR>

				<label>Item Name: <u><?php if($error_msg ==""){echo $row['item_name']; }?></u> </label>
		     	    	<BR><BR>
                            
				<label>Quantity: <u><?php if($error_msg ==""){echo $row['qty_borr']; }?></u> </label>
				<BR><BR>
		
				<label>Date Borrowed: <u><?php if($error_msg ==""){echo $row['date_borr']; }?></u> </label>
				<BR><BR>
...
-->
				...
				<table>
					<tr>	
						...
						<td>
              						<a href="student_pending.php" class="btn btn1">Cancel</a>
						</td>
					</tr>
				</table>
                    		</form>       
