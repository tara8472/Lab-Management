...
if ($_SERVER["REQUEST_METHOD"] == "POST") {	
	...
	if ($error_msg == ""){
		...
		if($result = mysqli_query($link, $sql)){
			if(mysqli_num_rows($result) > 0){
				...
				if($row['date_ret'] == "0000-00-00 00:00:00"){
	...
	try{
	if ($error_msg == ""){
	$sql = "INSERT INTO activity(user_id, user_name, item_id, item_name, date_borr, date_ret, qty_borr, teacher_appr, description) VALUES ('". $_SESSION["user_id"]   . "','". $_SESSION["user_name"] . "','". $_POST["item_id"]      . "','". $_POST["item_name"]    . "','". $date_borr    . "','0000-00-00 00:00:00',". $_POST["qty_borr"] . ",0, '')";
...
//			$error_msg = "Update Not Successful. Try again.";	
	...
	if ($error_msg == ""){
		header("Location: /labmanagement/student_borrow.php");
		exit ();
	}
}
		...
		<div class="wrapper">
			...
			<table>
				<tr>	
					...
					<td>
                        			<a href="student_borrow.php" class="btn btn1">Cancel</a>
					</td>
				</tr>
			</table>
                    </form>       
